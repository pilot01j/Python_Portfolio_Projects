...
SELECT_UPCOMING_MOVIES = "SELECT movies.* FROM movies WHERE release_timestamp > ?;"
SELECT_WATCHED_MOVIES = """SELECT id,title,release_timestamp FROM movies
...
def watch_movie(user_username, movie_id):
    with connection:
        connection.execute(INSERT_WATCHED_MOVIE, (user_username, movie_id))
